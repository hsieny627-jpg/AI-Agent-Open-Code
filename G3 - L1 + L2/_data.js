...
/* 名字的頭像（使用者 2026-09-25 指定：照老師提供的截圖裡的角色頭像）。
   把圖片放進這個資料夾的 avatars/（ken.png、mike.png、alan.png、wendy.png、emma.png，png／jpg／webp 都可以），
   重新 build 就會換成圖片；還沒放圖片的名字先用 emoji。 */
var AVDIR = require('path').join(__dirname, 'avatars');
function av(name, emoji) {
  var fs = require('fs');
  var ext = ['png', 'jpg', 'jpeg', 'webp'].filter(function (x) { return fs.existsSync(AVDIR + '/' + name + '.' + x); })[0];
  return ext ? '<img class="av" src="avatars/' + name + '.' + ext + '" alt="' + name + '">' : emoji;
}
/* 我／你的秒懂圖示（使用者 2026-09-25 指定：原本的 🙋 🙋🎒 👉 👉🎒 看不懂）
   左邊 🧒 ＝ 正在說話的人（頭上有 💬）＝「我」；右邊 👧 ＝ 聽的人 ＝「你」。
   亮的那一個就是這個字在說的人；「的」＝ 那個人身邊多一個書包 🎒（是他的東西）。 */
function pi(who, own) {
...
/* 替換字（使用者指定）：名字 5 個、歲數 6～12。放在卡片最下面，點一下整句（連發音）跟著換
   名字不翻譯（使用者 2026-09-25 指定）：中文那一格也寫英文名字 */
var SUB = {
  name: {
    basic:[['Ken','Ken',ICON.ken],['Alan','Alan',ICON.alan],['Wendy','Wendy',ICON.wendy],
           ['Mike','Mike',ICON.mike],['Emma','Emma',ICON.emma]],
    adv:[]
  },
  age: {
...
/* 替換字的按鈕上放不下 HTML 圖示：名牌、我／你的圖示只放在字卡上 */
SUB.name.basic.forEach(function (w) { if (/^</.test(w[2]) && !/<img/.test(w[2])) w[2] = ''; });
...
var W = {
  ...
  nm:function(en){return t(en,en,ICON[en.toLowerCase()],{slot:'name'})},
  age:function(en,zh,ic){return t(en,zh,ic,{slot:'age'})},
  ten:function(){return t('ten','十',ICON.ten,{slot:'age'})}
};
/* 「💡 問什麼？」的用法和動畫：跟四年級（sentences）Unit 1 第一張完全一樣（使用者 2026-09-25 指定照抄，暫時不放 Who） */
var USE = require('../sentences/_data.js').USE;
...
var U1 = [
 ...
 /* 5 中英語序：中文「什麼」在最後，英文 What 在最前面（名字改綠色，跟紅色的「什麼」分得開） */
 {type:'order', zhRow:[['你的',ICON.your,'b'],['名字',ICON.name,'gr'],['是',ICON.is,'y'],['什麼',ICON.what,'r'],['？','','g']],
  enRow:[['What',ICON.what,'r'],["'s",ICON.is,'y'],['your',ICON.your,'b'],['name',ICON.name,'gr'],['?','','g']],
  say:"What's your name?"},

 /* 6 回答 (1)：My name is ___. */
 {type:'sent', zh:'我的名字是 Ken。', slot:'name',
  tk:[W.My(),W.name(),W.is(),W.nm('Ken'),W.dot()]},

 /* 7 回答 (2)：I’m ___.（使用者 2026-09-25 指定：Mike 改成 Ken） */
 {type:'sent', zh:'我是 Ken。', slot:'name',
  tk:[W.I(),W.m(),W.nm('Ken'),W.dot()]},
 ...
 /* 13 問名字，要答名字（使用者指定：學生容易答錯的問答） */
 {type:'focus', title:'What’s your name? 要答什麼？',
  rows:[['My name is Ken.','✅ 名字',ICON.yes],['I’m Ken.','✅ 名字',ICON.yes],['I’m ten.','❌ 這是幾歲',ICON.no]]},

 /* 14 一問一答 (1)（使用者 2026-09-25 指定：Wendy 改成 Emma） */
 {type:'pair', cls:'b', slot:'name', qic:ICON.name, aic:ICON.emma,
  qtk:[W.What(),W.s(),W.your(),W.name(),W.q()],
  atk:[W.My(),W.name(),W.is(),W.nm('Emma'),W.dot()],
  qzh:'你的 名字 是 什麼？', azh:'我的 名字 是 Emma。'},
...
var U2 = [
 ...
 /* 10 縮寫變身：You are ➜ You’re（使用者指定的學習重點） */
 ...
 {type:'focus', title:'How old are you? 要答什麼？',
  rows:[['I’m ten years old.','✅ 幾歲',ICON.yes],['I’m ten.','✅ 幾歲',ICON.yes],
        ['I’m Mike.','❌ 這是名字',ICON.no],['I’m ten year old.','❌ years 要有 s',ICON.no]]},
...
var RV1 = [
 ...
 {t:'第 5～8 張', q:[
  ...
  {q:'I’m Ken. 的中文是？', o:['我是 Ken。','我的名字是什麼？','你是 Ken。','他是 Ken。'], h:'I ＝ 我，’m ＝ 是。'}
 ]},
 {t:'第 9～12 張', q:[
...
var SC1 = [
 ...
 sc({at:'🛝 操場',bg:'🛝',l:'👧',b:'What’s your name?',r:'👦',b2:'I’m Ken.',use:'更短的回答：I’m ＋ 名字。'}),
 SLOW('I am','I’m','說快一點，兩個字黏成一個字。'),
 SLOW('I am Ken.','I’m Ken.','意思一模一樣，只是說得比較快。'),
 sc({at:'🎤 自我介紹',bg:'💬',l:'🐢',ln:'說完整',b:'My name is Ken.',r:'🐇',rn:'最短',b2:'I’m Ken.',use:'三種說法，意思都一樣。'}),
...
var SC2 = [
 ...
 sc({at:'🎂 生日派對',bg:'🎈',l:'👩',b:'How old are you?',r:'👦',b2:'I’m ten years old.',use:'說自己幾歲：I’m ＋ 數字 ＋ years old。'}),
 SLOW('I am','I’m','說快一點，兩個字黏成一個字。'),
 SLOW('I am ten years old.','I’m ten years old.','意思一模一樣，只是說得比較快。'),
 ...
 sc({at:'✂️ 三句一樣',bg:'✂️',l:'🐢',ln:'說完整',b:'I am ten years old.',r:'🐇',rn:'最短',b2:'I’m ten.',use:'三種說法，意思都一樣。'}),
...
var CSS = '@media (max-aspect-ratio:1/1){.erow{flex-direction:column;gap:clamp(4px,.8vh,10px)}' +
  ...
  /* 我／你的圖示：兩個小朋友，亮的那一個就是這個字在說的人（負的左右邊界：圖示再寬也不會把字推開） */
  '.pi{display:inline-flex;align-items:flex-end;margin:0 -.45em;font-size:.82em;line-height:1}' +
  '.pi i{position:relative;font-style:normal;opacity:.22;filter:grayscale(1);transition:opacity .2s}' +
  '.pi i.on{opacity:1;filter:none}' +
  '.pi i em{position:absolute;left:-.35em;top:-.55em;font-style:normal;font-size:.55em}' +
