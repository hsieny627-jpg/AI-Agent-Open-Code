...
.ftw{display:flex;justify-content:center;align-items:flex-start;gap:.45em}
.ftc{display:flex;flex-direction:column;align-items:center}
.ftz{font-size:clamp(26px,4.5vh,44px);line-height:1.2;color:#F2F2F2;font-weight:700;letter-spacing:.06em;margin-top:.12em}
...
function draw(s){
 ...
 if(s.open||s.end){
  ...
  /* family 正下方是「家庭」、tree 正下方是「樹」（使用者 2026-09-24 指定） */
  h+='<div class="fen ftw in d1"><span class="ftc">{{family}}<span class="ftz">家庭</span></span>'+
     '<span class="ftc">{{tree}}<span class="ftz">樹</span></span></div>';
  if(s.zh&&s.zh!=='家庭樹')h+='<div class="fzh in d2">'+s.zh+'</div>';
  h+=tree(null);
 }else{
  h+=tree(s.k);
  h+=focus(s.k);
